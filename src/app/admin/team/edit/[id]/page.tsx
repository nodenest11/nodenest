...
export default function EditTeamMember() {
  ...
  useEffect(() => {
    async function fetchTeamMember() {
      ...
      try {
        const memberData = await TeamMemberService.getById(memberId);
        if (memberData) {
          setMember(memberData);
          setFormData({
            ...memberData,
            social: memberData.social || { twitter: "", linkedin: "", github: "", dribbble: "" }
          });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Update team member
      await TeamMemberService.update(memberId, formData);
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold">
            <span className="text-gradient">Edit Team Member</span>
          </h1>
          <p className="text-sm opacity-70 mt-1">Edit team member: {member?.name}</p>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center">
                <label htmlFor="order" className="block text-sm font-medium mr-2">Display Order:</label>
                <input
                  type="number"
                  id="order"
                  name="order"
